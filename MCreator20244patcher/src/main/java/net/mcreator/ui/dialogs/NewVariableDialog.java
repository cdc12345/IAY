...
public class NewVariableDialog {
	...
	private static VariableType last = new VariableType();
	...
	public static VariableElement showNewVariableDialog(MCreator mcreator, boolean showScope,
		...
		SearchableComboBox<VariableType> type = new SearchableComboBox<>(supportedTypes.toArray(new VariableType[0]));
		...
		if (supportedTypes.contains(last)){
			type.setSelectedItem(last);
		}
		...
		VariableElement element = getVariableElement(mcreator, showScope, inp, textField, type, scope);
		if (element != null) return element;
		return null;
	}

	private static VariableElement getVariableElement(MCreator mcreator, boolean showScope, JPanel inp, VTextField textField, JComboBox<VariableType> type, JComboBox<VariableType.Scope> scope) {
