...
public class VOptionPane {
	...
	public static String showInputDialog(Window frame, String text, String title, ImageIcon icon,
		...
		if (showDialog(frame, title, icon, ok, cancel, inp, textField)) return textField.getText();

		return null;
	}

	private static boolean showDialog(Window frame, String title, ImageIcon icon, String ok, String cancel, JPanel inp, VTextField textField) {
		...
		if (option == 0
			...
			return showDialog(frame, title, icon, ok, cancel, inp, textField);
		}
		return false;
	}
